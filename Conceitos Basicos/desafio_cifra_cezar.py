...
def retornaCifraCesar(texto, valor):
    novoTexto = list(texto)

    for i, caractere in enumerate(novoTexto):
        novoTexto[i] = chr(ord(novoTexto[i]) + valor)

    textoNovo = ''.join(novoTexto)
    return textoNovo

textoAntigo = 'Cachorro'
textoNovo = retornaCifraCesar(textoAntigo, -1)
print(textoNovo)